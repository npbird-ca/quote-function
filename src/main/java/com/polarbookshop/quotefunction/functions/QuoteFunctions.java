...
public class QuoteFunctions {
	...
	@Bean
	Function<Mono<MyData>, Mono<MyData>> transformMyData() {
		return mono -> mono.map(data -> {
			return new MyData(data.name() + "$", data.count + 1);
		});
	}
	...
	public record MyData (String name, int count) {
	}
}
